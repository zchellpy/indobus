...
const { data, status, refresh } = await useFetch<Route[]>('/api/routes', {
  key: 'table-routes',
  headers: useRequestHeaders(['cookie']),
  lazy: true
})

function getRowItems(row: Row<Route>) {
  return [
    {
      type: 'label',
      label: 'Actions'
    },
    {
      ...
      onSelect() {
        ...
        toast.add({
          ...
          description: 'Route ID copied to clipboard'
        })
      }
    },
    {
...
<template>
  <UDashboardPanel id="routes">
    <template #header>
      <UDashboardNavbar title="Daftar Rute" :ui="{ right: 'gap-4' }">
        <template #leading>
        ...
        <template #right>
          ...
          <RoutesSubmitModal :route="selectedRoute" :on-success="handleSuccessSubmit" />
        </template>
      </UDashboardNavbar>
    </template>

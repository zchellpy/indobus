...
type Rate = Database['public']['Tables']['expedition_rates']['Row']

const selectedRate = ref<Rate | null>(null)

const { data, status, refresh } = await useFetch<Rate[]>('/api/rates', {
  key: 'table-expedition_rates',
  headers: useRequestHeaders(['cookie']),
  lazy: true
})

function getRowItems(row: Row<Rate>) {
  return [
    {
      type: 'label',
      label: 'Actions'
    },
    {
      ...
      onSelect() {
        ...
        toast.add({
          ...
          description: 'Expedition Rate ID copied to clipboard'
        })
      }
    },
    {
      ...
      onSelect() {
        // selectedRate.value = { ...row.original }
        toast.add({
          title: 'Warning',
...
const columns: TableColumn<Rate>[] = [
  ...
  },
  {
    id: 'sub_category',
    header: 'Sub Kategori',
    cell: ({ row }) => row.original.sub_category ?? '-'
  },
  {
    id: 'base_price',
    header: 'Biaya',
...
<template>
  <UDashboardPanel id="rates">
    <template #header>
      <UDashboardNavbar title="Daftar Biaya Pengiriman" :ui="{ right: 'gap-4' }">
        <template #leading>
        ...
        <template #right>
          ...
          <RatesSubmitModal :rate="selectedRate" :on-success="handleSuccessSubmit" />
        </template>
      </UDashboardNavbar>
    </template>
